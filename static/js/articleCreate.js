...
async function validateInputs() {
	...
	if (!flag)
		alertMsg +=
			"1.태그를 제외한 입력칸 중 빈칸이 있거나 수량에 0보다 작거나 같은 수가 입력되었습니다.\n";
...
async function arrangeRecipeAndUpload(post = true) {
	...
	for (let i = 0; i < recipe_elements.length; i++) {
		...
		recipeImageContainer.setAttribute(
			"class",
			`recipe-image-${i + 1}-container recipe-image-container`
		);
		if (post) {
			...
			if (imageElement) {
				if (imageElement.getAttribute("id").includes("thumbnail"))
					imageElement.setAttribute("id", `recipe-${i + 1}-thumbnail`);
				if (imageElement.getAttribute("id").includes("url"))
					imageElement.setAttribute("id", `recipe-url-${i + 1}`);
			}
		}
	}
}
...
async function valuePropagate(ingredientNum) {
	...
	document.getElementById(`ingredient-unit-${ingredientNum}`).value =
		selectValue;
}
