...
async function loadRecommendation() {
	...
	moreBtn.setAttribute(
		"onclick",
		`location.href="${FRONT_BASE_URL}/users/feed.html"`
	);
	buttonBox.appendChild(moreBtn);
	if (!isLogin()) {
		document.getElementById("recomm-description").innerText =
			"당신만을 위한 추천레시피를 원하시면 로그인 해주세요!";
		...
		loginBtn.setAttribute(
			"class",
			"block btn btn-outline-secondary w-80 h-100 mt-5"
		);
		loginBtn.setAttribute(
			"onclick",
