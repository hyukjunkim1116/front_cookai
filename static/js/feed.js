async function renderList(recommendType,response_json){
	...
	for (const article of response_json) {
		...
		if (article.image) {
			articleimage.setAttribute("style", `background-image:url("${article.image}");`);
		} else {
			articleimage.setAttribute("style", `background-image:url("https://cdn11.bigcommerce.com/s-1812kprzl2/images/stencil/original/products/426/5082/no-image__12882.1665668288.jpg?c=2");`);
		}
		...
		newCardTitle.setAttribute("class", "article-content__title_col fs-6 text-truncate w-100");
...
async function loadCollavorativeRecommend(){
	if(!isLogin()){
		...
		newCardBox.parentElement.previousElementSibling.previousElementSibling.innerText="추천 피드"
		newCardBox.parentElement.previousElementSibling.innerText="추천 목록입니다. 당신만을 위한 추천목록은 로그인하셔야 이용가능합니다!"
...
async function loadFeedArticles(queryType,page=1){
	...
	if (!isLogin()) {
		...
		loginBtn.setAttribute("class","block btn btn-outline-secondary h-100 mt-5")
	...
	nextPageBtn.setAttribute("class","mt-2 btn btn-outline-success col-12")
	...
	if(response_json.next){
		...
		nextPageBtn.setAttribute("class","mt-2 btn btn-outline-secondary col-12")
